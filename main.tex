...
\begin{tikzpicture}[
    font=\small,
    stage/.style={draw=LineGray, rounded corners=2mm, thick, align=center, minimum width=3.05cm, minimum height=1.05cm, fill=SoftBlue},
    our/.style={draw=DeepBlue, rounded corners=2mm, very thick, align=center, minimum width=3.4cm, minimum height=1.2cm, fill=Mint},
    arr/.style={-{Latex[length=2.2mm]}, thick, draw=LineGray}
]
\node[stage] (rag) {普通 RAG\\文本向量召回};
\node[stage, right=0.85cm of rag] (grag) {GraphRAG\\图谱路径召回};
\node[stage, right=0.85cm of grag] (tkg) {Temporal KG\\时间状态建模};
\node[stage, right=0.85cm of tkg] (trust) {Trust Graph\\多源可信评估};
\node[our, below=1.0cm of tkg] (ours) {\projectshort{}\\时序可信图增强风险推理};
\draw[arr] (rag) -- (grag);
\draw[arr] (grag) -- (tkg);
\draw[arr] (tkg) -- (trust);
\draw[arr] (grag.south) -- (ours.north west);
\draw[arr] (tkg.south) -- (ours.north);
\draw[arr] (trust.south) -- (ours.north east);
\node[align=center, below=0.25cm of ours, text width=10.5cm] {融合多源采集、事件抽取、时序动态图谱、可信度量化、图拓扑计算、多 Agent 冲突消解与大模型研判生成。};
\end{tikzpicture}
...
\paragraph{一、文本抽取与报告生成算力。}事件抽取、摘要生成和研判报告生成可使用云端大模型 API、校内大模型服务或本地开源模型。若采用本地部署，建议配置单卡 24GB 以上显存用于 7B 至 14B 级模型推理；若需进行 LoRA 微调或批量抽取，建议使用 48GB 以上显存或多卡环境。竞赛演示阶段可采用“离线样例库 + 在线补充抽取”混合模式，降低实时算力压力。

\paragraph{二、向量检索与语义编码算力。}新闻文本、事件摘要、政策公告和证据片段需要编码为向量。该部分可使用轻量中文/英文双语嵌入模型，在 CPU 或单张中低端 GPU 上批量处理。由于项目数据规模以比赛演示和原型验证为主，百万级以下文本向量可通过单机向量数据库或轻量索引完成。
...
\midrule
开发工作站 & 用于代码开发、前端调试、数据清洗、图谱构建和本地演示。建议具备 16GB 至 32GB 内存、较高主频 CPU 和稳定网络。 \\
\midrule
GPU 服务器或云 GPU & 用于大模型本地推理、批量事件抽取、Embedding 编码和可能的 LoRA 微调。竞赛阶段可采用按需租用方式，降低成本。 \\
\midrule
...
\resizebox{\linewidth}{!}{
\begin{tikzpicture}[
    font=\footnotesize, % 全局字体降级为 footnotesize
    % 左侧层级节点尺寸等比缩减
    layer/.style={draw=LineGray, rounded corners=2mm, thick, align=center, text width=2.0cm, minimum height=0.7cm, fill=SoftBlue},
    % 右侧决策节点尺寸等比缩减
    decision/.style={draw=DeepBlue, rounded corners=2mm, very thick, align=center, text width=4.0cm, minimum height=0.9cm, fill=Mint},
    % 箭头尺寸同步缩小以适配新比例
    arr/.style={-{Latex[length=1.8mm]}, thick, draw=LineGray}
]
% 左侧输入层，垂直间距压缩至 0.2cm
\node[layer] (l1) {实体一致性};
\node[layer, below=0.2cm of l1] (l2) {事件一致性};
\node[layer, below=0.2cm of l2] (l3) {时间一致性};
\node[layer, below=0.2cm of l3] (l4) {来源一致性};
\node[layer, below=0.2cm of l4] (l5) {图谱一致性};

% 中间节点水平间距压缩至 0.8cm，并微调英文换行以适应变窄的文本盒
\node[decision, right=0.8cm of l3] (c) {综合置信度 $C(e)$\\confirmed / conflicting\\unverified / resolved};

% 尾部节点垂直间距压缩至 0.5cm
\node[decision, below=0.5cm of c] (risk) {风险评分与推荐排序\\高价值事件优先进入研判};

% 绘制连线
\foreach \x in {l1,l2,l3,l4,l5}{\draw[arr] (\x.east) -- (c.west);}
\draw[arr] (c.south) -- (risk.north);
\end{tikzpicture}
}
...
\begin{tikzpicture}[
    font=\small,
    layer/.style={draw=DeepBlue, rounded corners=2mm, very thick, align=center, minimum width=12.6cm, minimum height=0.9cm, fill=SoftBlue},
    sub/.style={draw=LineGray, rounded corners=1mm, align=center, minimum width=2.65cm, minimum height=0.58cm, fill=white},
    arr/.style={-{Latex[length=2.2mm]}, thick, draw=LineGray}
]
\node[layer] (l1) {1. 多源数据接入层：新闻 RSS、政策公告、制裁名单、市场价格、航运物流、气候灾害、社交舆情};
\node[layer, below=0.28cm of l1] (l2) {2. 事件抽取与结构化层：大模型抽取、实体消歧、字段校验、事件去重、样例库更新};
\node[layer, below=0.28cm of l2] (l3) {3. 时序动态图谱构建层：实体关系、事件节点、时间戳、有效期、状态机、历史版本};
\node[layer, below=0.28cm of l3] (l4) {4. 可信度评估与冲突消解层：来源信誉度、多源一致性、Trust Graph、Agent 辩论机制};
\node[layer, below=0.28cm of l4] (l5) {5. 风险传播推理与评分层：路径搜索、拓扑关键性、传播范围、风险分数、推荐排序};
\node[layer, below=0.28cm of l5] (l6) {6. 可视化展示与智能报告层：全球地图、产业链图谱、时间线、风险榜、证据面板、AI 报告};
\draw[arr] (l1) -- (l2);
\draw[arr] (l2) -- (l3);
\draw[arr] (l3) -- (l4);
\draw[arr] (l4) -- (l5);
...
\begin{itemize}
    ...
    \item 原文向量检索可在增强版中接入 Milvus、FAISS 或其他向量数据库；
...
\begin{itemize}
    ...
    \item \term{存储可升级}：原型阶段可使用本地文件和轻量数据库，后续可升级为 Neo4j、Milvus 等专业存储；
...
从工程实现角度看，系统可采用前后端分离架构。后端负责数据采集、事件抽取、图谱维护、风险评分和报告生成；前端负责全球风险地图、产业链图谱、风险时间线、节点风险榜单、证据链面板和 AI 研判报告展示。后端服务可使用 Python FastAPI、Node.js 或其他轻量 Web 框架实现，图谱存储在原型阶段可采用 JSON、SQLite 或本地轻量图结构，增强阶段可扩展为 Neo4j；向量检索在原型阶段可使用本地 Embedding 索引，后续可扩展为 FAISS、Milvus 或其他向量数据库。该路线既保证原型系统快速落地，又为后续工程升级保留足够空间。
...
当多源信息冲突时，系统触发多 Agent 冲突消解机制。不同 Agent 分别从官方证据、主流媒体、行业逻辑、市场数据和社交舆情角度对事件进行校验，并输出支持、反驳或不确定判断。冲突消解结果不简单删除争议信息，而是将其标记为 \texttt{confirmed}、\texttt{conflicting}、\texttt{unverified}、\texttt{resolved} 或 \texttt{invalidated}，供后续风险评分和报告生成使用。
