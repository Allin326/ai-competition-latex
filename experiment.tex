...
本章围绕\projectshort（以下简称\projectshort）系统进行全面的实验评估。与分类算法或回归模型的单一指标评测不同，\projectshort 是一个面向新能源锂电池产业链黑天鹅风险推理的端到端智能系统，包含多源数据接入、事件抽取、时序动态图谱构建、Trust Graph 可信度评估、图增强风险传播推理和大语言模型研判报告六个核心模块。因此，本章的实验设计遵循系统级评估范式：\term{提出假设→设计实验→验证假设→分析原因→得出结论}，而非简单堆叠 Accuracy/Precision/Recall 指标。
...
所有对比方法在以下维度保持严格统一：
\begin{itemize}
    \item \term{大语言模型}：统一使用 GPT-4o（2025-11-20 快照），Temperature = 0.1，Top-p = 0.9，Max Tokens = 4096；
    ...
    \item \term{向量数据库}：Milvus 2.4.0，索引类型 IVF\_FLAT（nlist = 128），度量方式余弦距离；
...
\begin{equation}
t = \frac{\bar{x}_d}{s_d / \sqrt{n}}, \quad
\bar{x}_d = \frac{1}{n}\sum_{i=1}^{n}(x_{i,\text{TIGER}} - x_{i,\text{Baseline}}), \quad
...
\textbf{配置项} & \textbf{参数} \\
\midrule
CPU & Intel Xeon Gold 6438M $\times$ 2，共 64 核 128 线程，2.0 GHz \\
GPU & NVIDIA A100-SXM-80GB $\times$ 1（LLM 推理）/ NVIDIA RTX 4090 24GB $\times$ 1（嵌入编码） \\
...
大语言模型 & GPT-4o（2025-11-20 快照），通过 API 调用 \\
...
Ground Truth 采用\textbf{双盲独立标注 + 专家仲裁}的流程：
\begin{enumerate}
    \item \term{第一轮}：两名标注员（均为新能源行业研究经验 2 年以上）分别独立对每个样本进行标注，标注内容包括事件类型、实体链接、风险等级、影响路径和有效时间窗口；
    \item \term{一致性检查}：计算两人标注的 Cohen's $\kappa$ 系数，$\kappa = 0.916$，表明标注一致性达到「几乎完美一致」水平；
    \item \term{仲裁}：对于不一致的样本（占 8.4\%），由第三位资深专家（行业经验 8 年以上）进行仲裁，确定最终标签。
\end{enumerate}
...
标注事件量 & 经过双盲标注和专家仲裁的风险事件总数 & 2,400 \\
事件类型覆盖 & 九类核心风险事件 & 9 \\
...
\begin{enumerate}
    ...
    \item \term{Baseline 2：Naive RAG（普通向量检索 RAG 基线）}。使用 \texttt{text-embedding-3-large} 将事件文本编码为向量，通过 Milvus 进行余弦相似度检索，将 Top-K 结果输入 LLM 生成回答。该基线代表当前主流的 RAG 问答范式。
...
测试集共 800 条标注样本，每条包含原始文本和标准事件 JSON。系统对每条原始文本执行结构化抽取，输出归一化事件对象。统计抽取结果与 Ground Truth 之间的字段完整率、事件类型准确率、实体匹配 F1 和时间抽取误差。为控制 LLM 调用成本，事件抽取使用 GPT-4o，单次抽取最大 Token 限制为 1024。
...
\begin{tabularx}{0.95\linewidth}{l c c c c c}
\toprule
...
\begin{tabularx}{0.95\linewidth}{l c c c c c}
\toprule
\textbf{方法} & \textbf{Latency P50 (ms)} & \textbf{Latency P95 (ms)} & \textbf{Latency P99 (ms)} & \textbf{TPS} & \textbf{Peak Mem (GB)} \\
\midrule
...
\begin{enumerate}
    ...
    \item \term{时序动态图谱映射}：系统将事件映射到产业链节点「nickel」（镍矿），并激活与此节点相关的下游边：「nickel $\xrightarrow{\text{供应}}$ nickel\_sulfate」「nickel\_sulfate $\xrightarrow{\text{材料}}$ ternary\_cathode」「ternary\_cathode $\xrightarrow{\text{制造}}$ battery\_cell」「battery\_cell $\xrightarrow{\text{应用}}$ ev」和「battery\_cell $\xrightarrow{\text{应用}}$ energy\_storage」。
...
为验证 H5，计算各组件的独立贡献和边际贡献。定义组件 $X$ 的独立贡献为 $\Delta_X = \text{Metric}_{\text{Full}} - \text{Metric}_{\text{w/o }X}$。
...
\toprule
\textbf{组件} & \textbf{独立贡献 $\Delta$ (Acc.)} & \textbf{独立贡献 $\Delta$ (Macro-F1)} & \textbf{独立贡献 $\Delta$ (EAP)} \\
\midrule
...
\node[box, text width=12cm, minimum height=9cm] {
...
\textbf{主要发现：}\\[0.3em]
• Accuracy 在 $\omega_1 \in [0.30, 0.40]$ 和 $\omega_2 \in [0.20, 0.30]$ 区域达到峰值（83.2--83.7\%）\\[0.2em]
• $\omega_1 < 0.20$ 时 Accuracy 下降至 79.1\%：严重度权重过低导致高严重度低关键性事件被低估\\[0.2em]
• $\omega_2 > 0.35$ 时 Accuracy 下降至 80.4\%：节点关键性权重过高导致低严重度高关键性事件被高估\\[0.2em]
• 当前默认权重（$\omega_1 = 0.35, \omega_2 = 0.25$）位于峰值区域\\[0.2em]
• Accuracy 在峰值区域的波动幅度不超过 $\pm 0.8\%$，说明评分模型对权重选择具有较好的鲁棒性
};
\end{tikzpicture}
\caption{风险评分权重敏感性分析}
...
\begin{itemize}
    ...
    \item \term{LLM 版本漂移}：GPT-4o 通过 API 调用，云端模型可能存在微小的版本更新。实验中使用 2025-11-20 快照版本并锁定，但若 API 提供方进行模型更新，本文的绝对指标可能无法精确复现。但相对比较的结论应仍然有效。
    \item \term{Embedding 模型依赖}：检索性能高度依赖 \texttt{text-embedding-3-large} 的质量。使用其他嵌入模型可能导致不同的绝对 Hit@K 值，但时序动态图谱相对于静态图谱的增益趋势应保持一致。
    \item \term{标注偏差}：尽管双盲标注的 Cohen's $\kappa = 0.916$ 已达「几乎完美一致」水平，但 Ground Truth 中的系统性偏差仍可能影响评估。标注员均来自新能源行业背景，对某些风险类型（如政策变化）可能比另一些类型（如价格异常）更为熟悉，导致标注质量标准不均衡。
\end{itemize}
